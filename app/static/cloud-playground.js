...
(function () {
    ...
    const KERNELS = {
        ...
        'break_rules': {
            category: 'push',
            label: 'Break the Rules On Purpose',
            blurb: 'ttsim is built to be stricter than real silicon. This trips one of its guardrails — and it is also a real, still-open limitation we found while building this Space.',
            ...
            code: _dedent(`
                ...
                # surfaces nonconformance before it becomes a silent, non-portable
                # correctness failure. This specific case (Blackhole, wide matmul) is
                # a genuine gap the simulator hasn't caught up to yet.
                import torch

                x = torch.randn(9, 768, dtype=torch.bfloat16)
                w = torch.randn(768, 3072, dtype=torch.bfloat16) * 0.02  # 768 -> 3072, MLP-width
